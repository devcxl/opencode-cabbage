...
import { computeWorkspaceDigest, type DigestOptions } from "../../../src/kernel/tdd/digest.js"
...
describe("computeWorkspaceDigest", () => {
  ...
  it("rejects symlinks that escape the worktree", async () => {
    ...
    // 注意：外部目标路径必须唯一（与 test/flowrun/digest.test.ts 的同名测试并行运行，避免固定路径冲突）
    const outsidePath = join(tmpdir(), `cabbage-digest-outside-${Math.random().toString(36).slice(2)}`)
